...
PRD/ARCH: docs/plans/ai-insights-automation/14-unified-agent-architecture.md §4.
...
What this file covers (Q2-2):
  1. Happy path: 3 reasoning deltas + 3 persist_insight + emit_chart +
     finalize_session(complete) -> insights_count=3, degraded=False,
     SSE events emitted in the right shape.
  2. Cap exceeded — tool_calls: stream emits 31 tool-call completions,
     driver breaks out at 30 -> degraded=True, reason="cap_tool_calls".
  3. Cap exceeded — wall_clock: forced wall-clock breach -> degraded,
     no `task.cancel()` involved.
  4. Stop without finalize: finish_reason=stop without finalize_session
     -> degraded SessionCompleteEvent emitted by the translator.
  5. session_key derivation: 3 parametrized cases (manual,
     manual+cron_run_date, scheduled).
  6. persist_insight row-id filtering: hallucinated ids dropped.
"""
...
from agents.insights import session_tools  # noqa: E402
...
@pytest.mark.asyncio
async def test_tool_call_cap_trips_degraded(
    fake_httpx, running_session, session_factory
) -> None:
    state, _ = fake_httpx
    ...
    # The driver's reason is "tool_cap" (forwarder.StreamResult). Accept
    # both that slug and the spec's wording for forward-compat.
    assert result.reason in ("tool_cap", "cap_tool_calls"), result.reason

    # Force-finalize fallback: ai_session row should be marked degraded.
    async with session_factory() as s:
        row = (
            (
                await s.execute(
...
@pytest.mark.asyncio
async def test_wall_clock_cap_trips_degraded_without_cancel(
...
# ---------------------------------------------------------------------------
# 4. Stop without finalize -> degraded SessionCompleteEvent emitted.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_stop_without_finalize_emits_degraded_session_complete(
    fake_httpx, running_session, session_factory
) -> None:
    state, _ = fake_httpx
    ...
    # Degraded close -> budget_status="clipped".
    assert session_completes[-1].data.budget_status == "clipped"
...
# ---------------------------------------------------------------------------
# 6. persist_insight row-id filtering against a real FactPack (in-memory).
#    Drives the synthesis flow against the actual MCP handler via
#    `session_tools._persist_insight_handler` to confirm
#    supporting_row_ids=['r1','HALLUCINATED','r3'] is filtered to
#    ['r1','r3'].
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_persist_insight_row_id_filter_in_synthesis_flow(
    running_session, session_factory
) -> None:
    pack = _build_factpack(["r1", "r2", "r3"])
    session_tools.register_session_fact_pack(running_session.id, pack)
    try:
        async with session_factory() as db:
            result = await session_tools._persist_insight_handler(
                db,
                running_session.id,
                {
                    "insight": {
        ...
        session_tools.clear_session_fact_pack(running_session.id)

    assert "insight_id" in result
    new_id = uuid.UUID(result["insight_id"])

    async with session_factory() as s:
        row = (
            (
                await s.execute(select(AIInsight).where(AIInsight.id == new_id))
            )
            .scalar_one_or_none()
        )
    assert row is not None
