"""
Iowa state permit / building data adapter — Phase 2 (AC9).

Iowa exposes its public datasets on data.iowa.gov via Socrata SODA. The
Iowa Utilities Board (IUB) and Iowa DNR publish facility / generator
permit datasets there. Without paid access, we hit the Socrata SODA JSON
endpoint with a $where clause filtering for data-center-relevant terms.

Default dataset is the Iowa "Generation Facilities" feed
(stable Socrata 4-4 ID: ekw8-w6yu). Override via IOWA_SODA_RESOURCE env
var.
...
_DEFAULT_SODA_RESOURCE = "8bwn-bk39"  # Iowa SPARS construction permits (DNR)
_IOWA_SODA_RESOURCE = os.environ.get("IOWA_SODA_RESOURCE", _DEFAULT_SODA_RESOURCE)
_IOWA_SODA_BASE = "https://data.iowa.gov/resource"
_PAGE_SIZE = 1000
...
class IowaPermitAdapter:
    adapter_name = "Iowa Open Data Permits"
    adapter_id = "iowa_open_data"
    adapter_version = "1.0.0"
    pillar = "building_permits"
    source_id = "iowa_open_data"
    declared_status = "partial"
    coverage_scope = "IA"

    def __init__(self, resource_id: str | None = None) -> None:
        self._resource_id = resource_id or _IOWA_SODA_RESOURCE
        self._url = f"{_IOWA_SODA_BASE}/{self._resource_id}.json"
    ...
    async def _fetch_page(self, offset: int) -> list[dict]:
        async with self._semaphore:
            ...
            if resp.status_code == 404:
                logger.warning("iowa_open_data.resource_not_found id=%s", self._resource_id)
                return []
    ...
    def _is_relevant(self, rec: dict) -> bool:
        ...
        if _DC_KEYWORDS.search(text_blob):
            return True
        # Fallback: any sufficiently large generator
        for k in ("capacity_mw", "nameplate_mw", "rated_capacity_mw", "mw"):
    ...
    def _normalize_record(self, rec: dict) -> dict | None:
        # Permit IDs vary by Iowa dataset — try common keys.
        permit_id = (
            rec.get("permit_number") or rec.get("permit_id")
            or rec.get("application_number") or rec.get("facility_id")
            or rec.get("project_id") or rec.get("id")
        )
        if not permit_id:
            return None
        permittee = (
            rec.get("applicant") or rec.get("owner") or rec.get("operator")
            or rec.get("company") or rec.get("facility_name") or ""
        )
        mw = (
            self._to_float(rec.get("capacity_mw"))
            or self._to_float(rec.get("nameplate_mw"))
            or self._to_float(rec.get("rated_capacity_mw"))
            or self._to_float(rec.get("mw"))
        )
        fuel = rec.get("fuel_type") or rec.get("fuel") or rec.get("technology") or ""
        status = rec.get("status") or rec.get("permit_status") or "filed"
        application_date = self._to_date(
            rec.get("application_date") or rec.get("filed_date")
            or rec.get("submission_date")
        )
        effective_date = self._to_date(
            rec.get("effective_date") or rec.get("issued_date")
            or rec.get("approval_date")
        )
        return {
            "source": self.source_id,
            "source_permit_id": f"IA-{str(permit_id).strip()}",
            "facility_name": str(rec.get("facility_name") or rec.get("project_name") or "").strip() or None,
    ...
    async def run(self, session: AsyncSession) -> dict:
        ...
        try:
            offset = 0
            all_rows: list[dict] = []
            for _page in range(20):  # cap at 20k rows
                try:
                    page = await self._fetch_page(offset)
                except httpx.HTTPStatusError as exc:
                    logger.error("iowa_open_data.fetch_http_error: %s",
                                 exc.response.status_code)
                    break
                except httpx.TransportError as exc:
                    logger.error("iowa_open_data.fetch_transport_error: %s", exc)
                    break
                if not page:
                    break
            ...
            logger.info("iowa_open_data.relevant_filtered total=%d relevant=%d",
            ...
            for entry in relevant:
                ...
                try:
                    ...
                    logger.error("iowa_open_data.upsert_error pid=%s err=%s",
            ...
            await session.flush()
            logger.error("iowa_open_data.run_fatal_error: %s", exc)
            raise
        finally:
            await self.close()
    ...
    async def _write_coverage(self, session: AsyncSession, n: int) -> None:
        stmt = pg_insert(DataCoverage).values(
            pillar=self.pillar, state_code="IA", source=self.source_id,
            coverage_status=self.declared_status, record_count=n,
            last_ingested_at=datetime.utcnow(), freshness_sla_hours=168,
            notes="Iowa Open Data (Socrata SODA) -- DC-relevant permits",
        )
        stmt = stmt.on_conflict_do_update(
    ...
    async def _write_lineage(self, session: AsyncSession, run_id: int, n: int) -> None:
        ...
        stmt = pg_insert(DataLineage).values(
            ...
            confidence=0.60,
            transformation={"method": "iowa_socrata_soda"},
        )
        await session.execute(stmt)
