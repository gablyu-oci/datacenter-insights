1) Executive summary
- Best improvement today: I stayed evidence-first and verified the operating surface before synthesizing. The clearest concrete error was an avoidable surface-assumption mistake (`read` on `/app/docs` as though it were a file), which is the same failure mode as schema guessing.
- Product understanding improved around the actual warehouse and runtime contracts: `events` is the milestone source of truth, `sites.power_capacity_mw` is the canonical MW field, `query_database` is single-statement/read-only/limited, and isolated cron runs use fresh sessions with guarded final-delivery behavior.
- The analyst/consultant takeaway is practical: separate tool-backed fact, doc/source-backed fact, and inference explicitly; then present the conclusion in executive order.
...
- From `QUERIES.md` and `AI_INSIGHTS_SQL_SCHEMA_DISCIPLINE.md`:
  - SQL must be schema-confirmed; plausible generic columns are a recurring failure mode.
...
- From `/app/docs/automation/cron-jobs.md`:
  - Cron runs inside Gateway, persists job definitions/state, and isolated jobs run in fresh sessions.
  - Isolated cron runs also guard against stale interim-status delivery and prefer final descendant output.
...
- `FRESHNESS.md` being sparse is not a reason to speculate; the fallback is schema-aware direct drilling.
...
Evidence reviewed
- Workspace: `MEMORY.md`, `SCHEMA.md`, `FRESHNESS.md`, `QUERIES.md`, `AI_INSIGHTS_PLAYBOOK.md`, `AI_INSIGHTS_SQL_SCHEMA_DISCIPLINE.md`, `AI_INSIGHTS_PREFLIGHT_CHECKLIST.md`, recent reflection archives
- OpenClaw docs: `/app/docs/AGENTS.md`, `/app/docs/automation/cron-jobs.md`, `/app/docs/brave-search.md`
- Memory: prior durable reflection lessons on schema/tool-contract discipline
